...
To perform this anlaysis, the R and Python programming languages [@r; @Python] as well as the following R and Python packages were used: `caret` [@caret], `docopt` [@docopt], `feather` [@feather], `tidyverse` [@tidyverse], `lubridate` [@lubridate], `car` [@car], `kableExtra` [@kableExtra], `reshape2` [@reshape2], `tidyverse` [@tidyverse], `broom` [@broom], `knitr` [@knitr], `ggpubr` [@ggpubr], `RCurl` [@RCurl], `here`[@here], `pandas` [@pandas], `numpy` [@numpy], `selenium` [@selenium], `scikit-learn` [@scikit-learn], and `altair` [@2018-altair], `magick`[@magick].
...
We also used ANOVA to calculate and interpret the features’ p-values, as ANOVA is a special case of linear model that assumes categorical predictors. This test will act as a validation for the categorical variables we determined as significant above. The results of our ANOVA test below confirms that the features `type`, `year`, `region`, and `month` are significant at a 0.05 significance level.

![](../doc/img/anova_table.png)

**Table 2.** Hypothesis test of significant features using ANOVA.

However, we should be cautious not to use the p-value significance as a stand alone measure to determine if these features are correlated with the target. We also conducted a multicollinearity test to determine if any of the features are redundant. We then used these results to inform which features should be included in the feature importances model.
...
Overall, there is fairly high collinarity between many of the predictors. This was expected, since they all deal with volume or number of avocados sold, be it by PLU code, bag type or total volume. In particular, `total_bags` and `total_volume` were expected to be highly correlated to other predictors that were sub-quantities of these totals. Due to the high correlation, including all these predictors in a prediction model would probably lead to overfitting.
...
kable(cv_scores,
      caption="**Table 4**. Cross-validation scores for each of the folds in the random forest regression model.")
```
...
kable(feat_imp,
      caption="**Table 5**. The relative feature importances of the top 15 most important features determined by random forest regression model.")
```
...
The `region` feature also seemed to play some importance in the pricing of avocados. For instance, regions such as Hartford-Springfield and San Francisco were the third and fourth most important predictors of average avocado price. It is unclear how these regions affect avocado prices.
