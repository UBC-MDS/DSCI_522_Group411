...
A random forest model was used to build a refression model to determine what the strongest predictors of avocado prices were. Before we fit the model, we first conducted a hypothesis test and a multicollinearity test to determine which features were significant and should be used in the model.
...
To perform this anlaysis, the R and Python programming languages [@r; @Python] as well as the following R and Python packages were used: caret [@caret], docopt [@docopt], feather [@feather], tidyverse [@tidyverse], lubridate [@lubridate], corr [@corr], car [@car], kableExtra [@kableExtra], reshape2 [@reshape2], tidyverse [@tidyverse], broom [@broom].
...
ADD RANDOM FOREST RESULT

# Discussion

ADD CRITIQUE OF RANDOM FOREST MODEL

Our random forest model may not be as accurate as it could be as we treated the month and region features as categorical. Month is ordinal and related to one another, thus it should be treated as a continuous variable. That being said, we could treat month as a categorical variable if we group the months into seasons and use season as a feature instead. Region is also related to each other as some regions are closer to one another than other regions. Therefore, we should transform region into continuous latitude and longitude variables.
