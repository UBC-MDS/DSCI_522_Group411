...
# Loading in the dataset 
avocado <- read_csv("src/avocado.csv")
```
...
avocado$month <- month(as.Date(avocado$Date), label=TRUE)
...
# Use the raw dataset
trainIndex_price2 <- createDataPartition(avocado$AveragePrice,
                                        p=0.8,
                                        list=FALSE,
                                        times=1)

priceTrain2 <- avocado[trainIndex_price2, ]
priceTest2 <- avocado[-trainIndex_price2, ]
...
```{r}
ggplot(priceTrain2, aes(x=region, y=AveragePrice)) +
...
price_per_region <- ggplot(priceTrain, aes(x=reorder(region, -AveragePrice), y=AveragePrice)) +
  geom_boxplot(fill="darkblue", alpha=0.1, width=0.2) +
...
price_per_type <- ggplot(priceTrain, aes(x=reorder(type, -AveragePrice), y=AveragePrice)) +
  geom_jitter(fill="darkblue", alpha=0.1, colour="darkblue", width=0.1) +
...
price_per_month <- ggplot(priceTrain, aes(x=month, y=AveragePrice, colour=type)) +
  geom_jitter(fill="darkblue", alpha=0.1, aes(colour=type)) +
...
```{r, include=FALSE}
write_csv(priceTrain, "avocado_train_clean.csv")
write_csv(priceTrain2, "avocado_train_raw.csv")
```
