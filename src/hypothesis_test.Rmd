...
Based on our EDA, we chose to fit a linear model to conudct our hypothesis test. To confirm that a linear model would be appropriate for this dataset, we will examine its residual plot. Looking at the residual plot below, the points are randomly distributed which indicates that a linear model is appropriate in this case.
...
ANOVA is a special case of linear model that assumes categorical predictors. We can also use ANOVA to calculate and interpret the features' p-values. This will act as a validation for the categorical variables we determined as significant above. The results of our ANOVA test below confirms that the features type, year, region, and month are significant at a 0.05 significance level.

```{r hypothesis test anova}
